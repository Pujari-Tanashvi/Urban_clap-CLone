var bestseller = [
  ...
  function navHighlighter() {
    ...
    sections.forEach((current) => {
      ...
      if (scrollY > sectionTop && scrollY <= sectionTop + sectionHeight) {
        document
          .querySelector("#productfilterbar a[href*=" + sectionId + "]")
  ...
  function displayPrices() {
    document.querySelectorAll('section').forEach(section => {
      const oldPriceElement = section.querySelector('.old-price');
      const newPriceElement = section.querySelector('.new-price');
  ...
  // Call the function to display prices correctly on page load
  displayPrices();
  if(total>0){
    totalprice();
  }
